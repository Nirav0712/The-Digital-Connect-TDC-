...
export const SubServiceShared = {
    Hero: ({ parentTitle, parentRoute, title, eyebrow, description, theme, visual: Visual }) => (
        <section className="relative pt-[120px] pb-24 lg:pt-[160px] lg:pb-32 px-6 lg:px-8 max-w-[1400px] mx-auto grid grid-cols-1 xl:grid-cols-2 gap-16 items-center">
            <div className="flex flex-col z-10 relative">
                ...
                <h1 className="text-4xl md:text-5xl lg:text-[64px] font-heading font-extrabold text-[#171717] leading-[1.05] mb-8 tracking-tight max-w-[800px]">{title}</h1>
                ...
                <div className="flex flex-wrap flex-col sm:flex-row items-start sm:items-center gap-4">
                    ...
                    <Link to="/portfolio" className="bg-transparent border border-black/10 text-[#171717] px-8 py-4 rounded-full font-semibold text-sm hover:border-black/30 transition-all flex items-center justify-center gap-2 w-full sm:w-auto">
                        View Work
                    </Link>
                </div>
            </div>
        ...
        <section className="py-24 lg:py-32 bg-white px-6 lg:px-8 relative overflow-hidden">
            <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 relative z-10">
                <div className="lg:col-span-5 relative">
                    ...
                    <h2 className="text-3xl lg:text-5xl font-heading font-extrabold text-[#171717] leading-tight sticky top-32">{title}</h2>
                </div>
                <div className="lg:col-span-7">
                    <div className="prose prose-lg prose-neutral max-w-none text-[#171717]/70 space-y-6 text-lg leading-relaxed">
            ...
            <div className="max-w-[1400px] mx-auto">
                <div className="mb-16 lg:mb-24 flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div className="max-w-2xl">
                        ...
                        <h2 className="text-3xl lg:text-5xl font-heading font-extrabold text-[#171717]">{title}</h2>
                    </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                    {capabilities.map((cap, i) => (
                        <div key={i} className="group bg-white border border-black/5 rounded-3xl p-8 lg:p-10 hover:-translate-y-2 hover:shadow-[0_20px_40px_-20px_rgba(0,0,0,0.1)] transition-all duration-500 relative overflow-hidden">
                            ...
                            <h3 className="text-xl font-bold text-[#171717] mb-4 group-hover:text-black transition-colors">{cap.title}</h3>
                            <p className="text-[#171717]/60 leading-relaxed text-sm md:text-base">{cap.desc}</p>
                        </div>
                    ))}
                </div>
            </div>
    ...
    Technology: ({ technologies, theme, title = "Technology Stack", eyebrow = "Engineering Core" }) => (
        <section className="py-24 bg-white px-6 lg:px-8 border-y border-black/5 overflow-hidden relative">
            <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row gap-12 lg:gap-24 relative z-10">
                <div className="w-full lg:w-1/3 shrink-0">
                    <span className={"text-[13px] font-bold tracking-[0.2em] uppercase mb-4 block " + theme.accent}>{eyebrow}</span>
                    <h2 className="text-3xl lg:text-4xl font-heading font-extrabold text-[#171717] mb-6">{title}</h2>
                    <p className="text-[#171717]/60 text-lg">We utilize industry-leading tools and frameworks to ensure stability, speed, and scalability for every project we deploy.</p>
                </div>
                <div className="w-full lg:w-2/3 flex flex-wrap gap-4 h-fit content-start">
                    {technologies.map((tech, i) => (
                        <div key={i} className={"px-6 py-3 rounded-xl border border-black/5 text-[15px] font-semibold bg-white shadow-sm hover:scale-105 hover:border-black/20 hover:shadow-md transition-all cursor-default flex items-center gap-2 " + theme.accent}>
                            {tech}
                        </div>
                    ))}
                </div>
    ...
    Process: ({ process, theme, title = "Methodology", eyebrow = "How We Work" }) => (
        <section className="py-24 lg:py-32 px-6 lg:px-8 bg-white border-b border-black/5 relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12 relative">
                    ...
                    {process.map((step, i) => (
                        <div key={i} className="relative group flex flex-col items-center text-center lg:items-start lg:text-left">
                            <div className={"w-14 h-14 rounded-full flex items-center justify-center font-bold text-lg mb-8 bg-white border-2 border-black/5 relative z-10 group-hover:scale-110 group-hover:border-black/20 transition-all shadow-sm " + theme.accent}>
                                {i + 1}
                            </div>
                            <h3 className="text-xl font-bold text-[#171717] mb-4">{step.title}</h3>
            ...
            <div className="max-w-[1400px] mx-auto">
                <div className="mb-16 flex flex-col lg:flex-row gap-8 justify-between items-start">
                    <div className="max-w-xl">
                        ...
                        <h2 className="text-3xl lg:text-4xl font-heading font-extrabold text-[#171717]">Why This Matters</h2>
                    </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {benefits.map((ben, i) => (
                        <div key={i} className="flex gap-6 p-8 rounded-3xl bg-white border border-black/5 shadow-sm hover:shadow-md transition-shadow">
                            <div className={"w-12 h-12 rounded-full shrink-0 flex items-center justify-center font-bold " + theme.softBg + " " + theme.accent}>
                                ${(i + 1)}
                            </div>
                            <div>
                                <h3 className="text-xl font-bold text-[#171717] mb-3">{ben.title}</h3>
                                <p className="text-[#171717]/60 leading-relaxed">{ben.desc}</p>
                            </div>
                        </div>
                    ))}
                </div>
    ...
    FAQ: ({ faqs, theme }) => (
        <section className="py-24 px-6 lg:px-8 bg-white border-y border-black/5">
            <div className="max-w-[1000px] mx-auto">
                ...
                <div className="space-y-4">
                    {faqs.map((faq, i) => (
                        <details key={i} className="group border border-black/10 rounded-2xl bg-white overflow-hidden">
                            ...
                            <p className="text-[#171717]/60 px-6 pb-6 md:px-8 md:pb-8 leading-relaxed">
                                {faq.a}
                            </p>
                        </details>
                    ))}
        ...
        <section className="py-24 px-6 lg:px-8 bg-[#FAF9F7]">
            <div className="max-w-[1400px] mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {links.map((link, i) => (
                        <Link key={i} to={link.href} className="group bg-white border border-black/5 rounded-2xl p-6 md:p-8 hover:-translate-y-2 hover:shadow-[0_15px_30px_-15px_rgba(0,0,0,0.1)] transition-all duration-300 block relative overflow-hidden">
                            <h3 className="text-xl font-bold text-[#171717] mb-4 pr-8 relative z-10">{link.title}</h3>
                            <div className={"absolute bottom-6 right-6 w-10 h-10 rounded-full flex items-center justify-center border border-black/10 group-hover:border-transparent transition-all z-10 " + theme.bg + " " + theme.accent}>
    ...
    CTA: ({ theme }) => (
        <section className={"py-32 px-6 lg:px-8 relative overflow-hidden " + theme.softBg}>
            ...
            <div className="max-w-[1000px] mx-auto text-center flex flex-col items-center relative z-10">
                <h2 className="text-4xl md:text-5xl lg:text-6xl font-heading font-extrabold text-[#171717] mb-8 tracking-tight">Have a project in mind?</h2>
                <p className="text-xl text-[#171717]/70 leading-relaxed max-w-2xl mb-12">
                    Let's build something meaningful. Tell us what you're building and we'll engineer the solution collaboratively.
                </p>
                <div className="flex flex-col sm:flex-row gap-4">
                    <Link to="/contact" className="bg-[#171717] text-white px-10 py-5 rounded-full font-bold text-sm tracking-wide uppercase hover:bg-black transition-all shadow-[0_10px_20px_-10px_rgba(0,0,0,0.3)] hover:-translate-y-1 flex items-center justify-center gap-3 w-full sm:w-auto">
                        Start a Project <ArrowRight className="w-5 h-5" />
                    </Link>
                    <Link to="/services" className="bg-white border-2 border-black/5 text-[#171717] px-10 py-5 rounded-full font-bold text-sm tracking-wide uppercase hover:border-black/20 hover:bg-[#FAF9F7] transition-all flex items-center justify-center gap-3 w-full sm:w-auto">
                        Explore Services
                    </Link>
                </div>
            </div>

...
const Footer = () => {
  return (
    <footer className="bg-brand-bg-dark text-white pt-24 pb-12 px-6 overflow-hidden relative tech-grid-dark">
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-brand-deep-blue/20 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-brand-cyan/5 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/3 pointer-events-none"></div>

      {/* Decorative Gold Accent Line */}
      <div className="absolute top-0 left-12 w-24 h-1 bg-brand-gold/80 rounded-b-lg"></div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="pt-8 border-t border-brand-cyan/20 flex flex-col md:flex-row justify-between items-center gap-4 text-[#B9CAD7]/60 text-sm">
          <p>&copy; {new Date().getFullYear()} The Digital Connect. All rights reserved.</p>
          <div className="flex gap-6">
            <Link to="/privacy-policy" className="hover:text-brand-electric-cyan transition-colors">Privacy Policy</Link>
            <Link to="/terms-and-conditions" className="hover:text-brand-electric-cyan transition-colors">Terms of Service</Link>
          </div>
        </div>
      </div>
    </footer>
